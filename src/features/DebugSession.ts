    ...
    implements DebugConfigurationProvider, DebugAdapterDescriptorFactory
{
    ...
    public async provideDebugConfigurations(
        ...
        const debugConfigPickItems = [
            ...
            },
            {
                ...
                label: "Attach",
                description:
                    "Attach the debugger to a running PowerShell Host Process",
            },
            {
    ...
    public async createDebugAdapterDescriptor(
        ...
        return new DebugAdapterNamedPipeServer(
    ...
    private async createTemporaryIntegratedConsole(
        ...
        if (session.configuration.attachDotnetDebugger) {
            ...
            const startDebugEvent = debug.onDidStartDebugSession(
                (dotnetAttachSession) => {
                    if (
                        dotnetAttachSession.configuration.name !=
                    ...
                    const stopDebugEvent = debug.onDidTerminateDebugSession(
                        async (tempConsoleSession) => {
                            if (
                                tempConsoleDotnetAttachSession.parentSession
                                    ?.id !== tempConsoleSession.id
                            ) {
                                return;
                            }
    ...
    private async resolveAttachDebugConfiguration(
        ...
        if (
            config.processId === "current" ||
            config.processId ===
                (await this.sessionManager.getLanguageServerPid())
        ) {
        ...
        return {
            ...
            onWillReceiveMessage: (m): void => {
                ...
                if (
                    ...
                    this.log?.trace(
                        `${m.seq}: ` +
                            JSON.stringify(m.arguments, undefined, 2),
                    );
                }
            },
            onDidSendMessage: (m): void => {
                ...
                if (
                    ...
                    this.log?.trace(
                        `${m.seq}: ` + JSON.stringify(m.body, undefined, 2),
                    );
                }
            },
...
export class SpecifyScriptArgsFeature implements Disposable {
    ...
    private async specifyScriptArguments(): Promise<string | undefined> {
        ...
        const options: InputBoxOptions = {
            ignoreFocusOut: true,
            placeHolder:
                "Enter script arguments or leave empty to pass no args",
        };
...
export const GetRunspaceRequestType = new RequestType<
    IGetRunspaceRequestArguments,
    IRunspace[],
    string
