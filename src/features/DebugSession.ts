...
export const StopDebuggerNotificationType = new NotificationType<void>(
...
type ResolveDebugConfigurationResult =
  | DebugConfiguration
  | PREVENT_DEBUG_START
  | PREVENT_DEBUG_START_AND_OPEN_DEBUGCONFIG;
...
export const DebugConfigurations: Record<DebugConfig, DebugConfiguration> = {
  ...
  [DebugConfig.LaunchScript]: {
    ...
    request: "launch",
    script:
      'Enter path or command to execute, for example: "${workspaceFolder}/src/foo.ps1" or "Invoke-Pester"',
    args: [],
  },
  ...
  [DebugConfig.ModuleInteractiveSession]: {
    ...
    request: "launch",
    script:
      'Enter command to import your binary module, for example: "Import-Module -Force ${workspaceFolder}/path/to/module.psd1|dll"',
  },
  ...
  implements DebugConfigurationProvider, DebugAdapterDescriptorFactory
{
      ...
      languageClient.onNotification(
        ...
        () =>
          void debug.startDebugging(
            undefined,
            DebugConfigurations[DebugConfig.InteractiveSession],
          ),
      ),
  ...
  public async provideDebugConfigurations(
    ...
    const debugConfigPickItems = [
      ...
      },
      {
        ...
        description:
          "Debug commands executed from the PowerShell Extension Terminal",
      },
      {
      ...
      },
      {
        ...
        description:
          "Debug Pester Tests detected in your current directory (runs Invoke-Pester)",
      },
      {
        id: DebugConfig.ModuleInteractiveSession,
        label: "Interactive Session (Module)",
        description:
          "Debug commands executed from the PowerShell Extension Terminal after auto-loading your module",
      },
      {
    ...
    const launchSelection = await window.showQuickPick(debugConfigPickItems, {
  ...
  public async resolveDebugConfiguration(
    ...
    _token?: CancellationToken,
  ): Promise<ResolveDebugConfigurationResult> {
  ...
  public async createDebugAdapterDescriptor(
    ...
    const sessionDetails = session.configuration
    ...
    this.logger.writeDebug(
      `Debug configuration: ${JSON.stringify(session.configuration, undefined, 2)}`,
    );
  ...
  private async resolveLaunchDebugConfiguration(
    ...
    if (config.current_document) {
      ...
      if (await checkIfFileExists(config.script)) {
        ...
        if (!(ext === ".ps1" || ext === ".psm1")) {
          void this.logger.writeAndShowError(
            `PowerShell does not support debugging this file type: '${path.basename(config.script)}'.`,
          );
          return PREVENT_DEBUG_START_AND_OPEN_DEBUGCONFIG;
        }
      }
  ...
  private resolveAttachDotnetDebugConfiguration(
    config: DebugConfiguration,
  ): ResolveDebugConfigurationResult {
  ...
  private async createTemporaryIntegratedConsole(
    ...
    if (session.configuration.attachDotnetDebugger) {
      ...
      const startDebugEvent = debug.onDidStartDebugSession(
        (dotnetAttachSession) => {
          if (
            dotnetAttachSession.configuration.name != dotnetAttachConfig.name
          ) {
            return;
          }
          ...
          const stopDebugEvent = debug.onDidTerminateDebugSession(
            async (tempConsoleSession) => {
              if (
                tempConsoleDotnetAttachSession.parentSession?.id !==
                tempConsoleSession.id
              ) {
                return;
              }
      ...
      this.logger.writeDebug(
        `Dotnet attach debug configuration: ${JSON.stringify(dotnetAttachConfig, undefined, 2)}`,
      );
  ...
  private getDotnetNamedConfigOrDefault(
    configName?: string,
  ): ResolveDebugConfigurationResult {
    if (configName) {
      ...
      return debugConfigs.find(
        ({ type, request, name }) =>
          type === "coreclr" && request === "attach" && name === configName,
      );
    }
...
class PowerShellDebugAdapterTrackerFactory
  implements DebugAdapterTrackerFactory, Disposable
{
  ...
  get log(): LogOutputChannel | undefined {
    if (
      ...
      this._log = window.createOutputChannel(`${this.adapterName}: Trace DAP`, {
        log: true,
      });
  ...
  createDebugAdapterTracker(session: DebugSession): DebugAdapterTracker {
    ...
    return {
      ...
      onExit: (code) =>
        this.log?.info(`${sessionInfo} exited with code ${code}`),
      onWillReceiveMessage: (m): void => {
        this.log?.debug(`➡️${m.seq} ${m.type}: ${m.command}`);
        if (
          m.arguments &&
          (Array.isArray(m.arguments)
            ? m.arguments.length > 0
            : Object.keys(m.arguments).length > 0)
        ) {
          this.log?.trace(
            `${m.seq}: ` + JSON.stringify(m.arguments, undefined, 2),
          );
        }
      },
      onDidSendMessage: (m): void => {
        const responseSummary =
          m.request_seq !== undefined
...
export const GetPSHostProcessesRequestType = new RequestType<
  IGetPSHostProcessesArguments,
  IPSHostProcessInfo[],
  string
